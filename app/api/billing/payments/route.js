...
export async function POST(request) {
    try {
        ...
        // Strategy: Find most recent PENDING invoice for this user
        // If month/year are provided in body, use them. Otherwise, find ANY pending invoice.
        ...
        // First, try to find invoice with exact month/year match (if provided)
        let existingIndex = -1;

        if (targetMonth !== null && targetYear !== null) {
            existingIndex = payments.findIndex(p =>
                p.username === body.username &&
                parseInt(p.month) === targetMonth &&
                parseInt(p.year) === targetYear
            );
        }

        // If not found and no month/year specified, find most recent PENDING invoice for user
        if (existingIndex === -1) {
            const userPendingInvoices = payments
                .map((p, index) => ({ ...p, originalIndex: index }))
                .filter(p => p.username === body.username && p.status !== 'completed')
                .sort((a, b) => new Date(b.date) - new Date(a.date));

            if (userPendingInvoices.length > 0) {
                existingIndex = userPendingInvoices[0].originalIndex;
            }
        }
        ...
        if (existingIndex !== -1) {
            // Update existing invoice to completed
            payments[existingIndex] = {
                ...payments[existingIndex],
                date: new Date().toISOString(),
                status: 'completed',
                amount: amount,
                method: body.method || payments[existingIndex].method || 'cash',
                notes: body.notes || payments[existingIndex].notes,
                commissions: commissions
            };
            newPayment = payments[existingIndex];
        } else {
            // Create new invoice only if no pending invoice exists
            newPayment = {
        ...
        try {
            ...
            const notifMessage = `"System" : Payment of ${currencyFormatter.format(Number(newPayment.amount))} for Invoice ${newPayment.invoiceNumber} received.`;
            // Add username explicitly to ensure it's filtered correctly
            await addNotification(notifMessage, { username: newPayment.username });
        } catch (notifError) {
