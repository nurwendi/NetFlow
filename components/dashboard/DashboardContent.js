...
import PartnerBillingPage from '@/app/billing/partner/page';
...
export default function DashboardContent() {
    ...
    }, [stats]); // stats in dependency might cause loop if not careful, but spread ...stats helps. Actually better to remove stats from dependency or act differently. 
    // Fix: Remove `stats` from dependency array and use functional updates if needed, OR just trust `newStats = {...stats}` copies current state at execution time.
    // Actually `stats` dependency is bad here. Removing it.
    ...
    return (
        <>
            <motion.div
                className="space-y-8 pb-24" // Extra padding for customizer button
            ...
            >
                ...
                {/* Widgets */}
                <AnimatePresence>
                    {visibleWidgets.financial && userRole !== 'partner' && (
                    ...
                    {visibleWidgets.tempChart && (
                        <TemperatureChart key="tempChart" data={temperatureHistory} />
                    )}

                    {visibleWidgets.cpuChart && (
                        <CpuChart key="cpuChart" data={cpuHistory} />
                    )}

                    {visibleWidgets.trafficChart && (
                        <TrafficHistoryChart key="trafficChart" data={trafficData} />
                    )}
                </AnimatePresence>
            </motion.div>
